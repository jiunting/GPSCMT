...
def makeD(new_stations,Loc_col=[0,1],use_Dcol=[2,3,4],err=[5,6,7]):
    ...
    all_comp=np.array(all_comp) #now it should be lon,lat,E,N,U,sE,sN,sU
    #stack the data
    stacked_D=[]
...
def GMD_solve(G,D):
    ...
    res=(np.mean( (D-np.dot(G,M))**2 ))**0.5
    return M,res
...
def run_inv(ENUdir,all_G,GRDfile,STA,data_file,outlog):
#def run_inv(ENUdir,GRDfile,STA,data_file,outlog): #don't want to preload
    '''
        ENUdir: the directory of the *ENU file
        all_grids: pre-loaded G
        GRDfile: path of the GRD that generated GFs
    ...
    all_idx,new_stations=find_STAidx(STA,stations,name_col,LL_col[0],LL_col[1],[121,23],10) #find the id in the G matrix, new_stations:new station file without issued stations
    #make the D array
    ...
    if not(all_G):
        ...
        if len(all_G)==len(GRD): #quick check if the preload_G input is right
            ...
            for i,Gname in enumerate(all_grids):
                all_G[i]=np.load(Gname)
    '''
    ngrids=['/Users/timlin/Documents/Project/GPSInv/GFs/TW_dense/ENU_out/SRC%05d_ENU.npy'%(ii+1) for ii in range(57834)]

    #sav_res=[]
    #sav_M=[]
    minres=1e20
    #for ng in all_G.keys(): #pre-loaded G
    for ng in range(len(ngrids)): #save memory, load G when your turn
    #for ng,ngrid in enumerate([all_grids[26969]]):
        #if ng%1000==0:
            #print('First search:',ng,'out of',len(ngrids))
        #print(all_idx)
        #G=makeG(ngrid=ngrids[ng],use_idx=all_idx,use_comp=comp_INV)
        G=makeG_dict(all_G[ng],all_idx,comp_INV)
        #weighted LSQ
        new_G=[]
    ...
    OUT_inv.write('EQloc: %f %f %f\n'%(GRD[sav_ng,0],GRD[sav_ng,1],GRD[sav_ng,2]))
...
for n_dir in range(3):
    time1=time.time()
    data_file='/Users/timlin/Documents/Project/GPSInv/code/Inversion/TEST%05d/SRC_sum.enu'%(n_dir)
    print('working on:',data_file)
    outlog='/Users/timlin/Documents/Project/GPSInv/code/Inversion/TEST%05d/Inv_out_2.log'%(n_dir)
    try:
        run_inv(ENUdir,all_G,GRDfile,STA,data_file,outlog)# preload G
        #run_inv(ENUdir,GRDfile,STA,data_file,outlog)
